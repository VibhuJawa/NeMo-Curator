...
Install the dedicated `nemotron_ocr` extra on a Linux system with an NVIDIA GPU:

```bash
uv pip install "nemo-curator[nemotron_ocr]"
```

From a source checkout, use `uv sync --extra nemotron_ocr`. This installs a `nemotron-ocr` release compatible with Curator's pinned PyTorch version; installing `sdg_cuda12` or `image_cuda12` alone does not install it.

Alternatively, use the NeMo Curator container, which includes the extension:
...
The optional verifier reads `NVIDIA_API_KEY` from each worker's environment during setup. The legacy `NVINFERENCE_API_KEY` remains supported for compatibility. Do not place the key in source, CLI arguments, or serialized stage configuration. Obtain a key from [NVIDIA Build](https://build.nvidia.com/settings/api-keys) and ensure Ray workers inherit it.
...
`NVInferenceClient` uses the NVIDIA integration endpoint, reads `NVIDIA_API_KEY` (with `NVINFERENCE_API_KEY` as a compatibility fallback), streams final-answer content, allows 10 concurrent requests per worker, uses a 120-second timeout, and retries rate-limit and connection failures three times with exponential backoff and jitter.
