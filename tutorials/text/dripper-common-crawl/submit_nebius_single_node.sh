...
REQUEST_MANIFEST_INFERENCE_ONLY="${REQUEST_MANIFEST_INFERENCE_ONLY:-0}"
...
LAYOUT_TEMPLATE_ACCEPTANCE_SIGNATURE_MODE="${LAYOUT_TEMPLATE_ACCEPTANCE_SIGNATURE_MODE:-none}"
...
LAYOUT_TEMPLATE_SPLIT_EXTERNAL_INFERENCE="${LAYOUT_TEMPLATE_SPLIT_EXTERNAL_INFERENCE:-0}"
...
LAYOUT_TEMPLATE_PROMPT_DEDUP_FALLBACK_MIN_FRACTION="${LAYOUT_TEMPLATE_PROMPT_DEDUP_FALLBACK_MIN_FRACTION:-0}"
LAYOUT_TEMPLATE_MIN_SAVED_CALL_PAGES="${LAYOUT_TEMPLATE_MIN_SAVED_CALL_PAGES:-0}"
...
echo "  Boundary  : request_manifest_inference_only=${REQUEST_MANIFEST_INFERENCE_ONLY}"
...
echo "  Layout    : template=${LAYOUT_TEMPLATE_MODE} layout_id_col=${LAYOUT_TEMPLATE_LAYOUT_ID_COL:-none} precompute_layout_ids=${LAYOUT_TEMPLATE_PRECOMPUTE_LAYOUT_IDS} baseline=${LAYOUT_BASELINE_OUTPUT_DIR:-none} threshold=${LAYOUT_CLUSTER_THRESHOLD} signature=${LAYOUT_PAGE_SIGNATURE_MODE} exact_query_keys=${LAYOUT_EXACT_QUERY_VALUE_KEYS} failed_host_signature=${LAYOUT_TEMPLATE_FAILED_HOST_FALLBACK_SIGNATURE_MODE} failed_layout_signature=${LAYOUT_TEMPLATE_FAILED_LAYOUT_FALLBACK_SIGNATURE_MODE} min_cluster=${LAYOUT_TEMPLATE_MIN_CLUSTER_SIZE} fallback_llm=${LAYOUT_TEMPLATE_FALLBACK_LLM} defer_fallback_llm=${LAYOUT_TEMPLATE_DEFER_FALLBACK_LLM} split_external_inference=${LAYOUT_TEMPLATE_SPLIT_EXTERNAL_INFERENCE} require_success=${LAYOUT_TEMPLATE_REQUIRE_SUCCESS} max_selected_ratio=${LAYOUT_TEMPLATE_MAX_SELECTED_ITEM_RATIO} max_rep_selected_ratio=${LAYOUT_TEMPLATE_MAX_REP_SELECTED_ITEM_RATIO} min_main_html_sim=${LAYOUT_TEMPLATE_MIN_MAIN_HTML_SIM:-default} content_len_ratio=${LAYOUT_TEMPLATE_MIN_CONTENT_LENGTH_RATIO:-default}:${LAYOUT_TEMPLATE_MAX_CONTENT_LENGTH_RATIO:-default} more_noise=${LAYOUT_TEMPLATE_MORE_NOISE_ENABLE} validation_rows=${LAYOUT_TEMPLATE_VALIDATION_ROWS} validation_min_f1=${LAYOUT_TEMPLATE_VALIDATION_MIN_CONTENT_F1} validation_signature=${LAYOUT_TEMPLATE_VALIDATION_SIGNATURE_MODE} acceptance_signature=${LAYOUT_TEMPLATE_ACCEPTANCE_SIGNATURE_MODE} large_validation_rows=${LAYOUT_TEMPLATE_LARGE_CLUSTER_VALIDATION_ROWS} large_min_size=${LAYOUT_TEMPLATE_LARGE_CLUSTER_MIN_SIZE} representative_candidates=${LAYOUT_TEMPLATE_REPRESENTATIVE_CANDIDATES} feature_source=${LAYOUT_TEMPLATE_FEATURE_SOURCE} propagation_target=${LAYOUT_TEMPLATE_PROPAGATION_TARGET} propagation_content_source=${LAYOUT_TEMPLATE_PROPAGATION_CONTENT_SOURCE} host_single_min=${LAYOUT_TEMPLATE_HOST_SINGLE_CLUSTER_MIN_PAGES} host_single_max=${LAYOUT_TEMPLATE_HOST_SINGLE_CLUSTER_MAX_PAGES} max_exact_host_pages=${LAYOUT_TEMPLATE_MAX_EXACT_HOST_PAGES} large_host_mode=${LAYOUT_TEMPLATE_LARGE_HOST_MODE} prompt_dedup_fallback_min_fraction=${LAYOUT_TEMPLATE_PROMPT_DEDUP_FALLBACK_MIN_FRACTION} min_saved_call_pages=${LAYOUT_TEMPLATE_MIN_SAVED_CALL_PAGES} propagation_concurrency=${LAYOUT_TEMPLATE_PROPAGATION_CONCURRENCY}"
...
if [ "${LAYOUT_TEMPLATE_SPLIT_EXTERNAL_INFERENCE}" = "1" ]; then
    extra_args+=(--layout-template-split-external-inference)
else
    extra_args+=(--no-layout-template-split-external-inference)
fi
...
extra_args+=(--layout-template-max-rep-selected-item-ratio "${LAYOUT_TEMPLATE_MAX_REP_SELECTED_ITEM_RATIO}")
...
extra_args+=(--layout-template-acceptance-signature-mode "${LAYOUT_TEMPLATE_ACCEPTANCE_SIGNATURE_MODE}")
...
extra_args+=(--layout-template-feature-source "${LAYOUT_TEMPLATE_FEATURE_SOURCE}")
extra_args+=(--layout-template-propagation-target "${LAYOUT_TEMPLATE_PROPAGATION_TARGET}")
extra_args+=(--layout-template-propagation-content-source "${LAYOUT_TEMPLATE_PROPAGATION_CONTENT_SOURCE}")
...
extra_args+=(--layout-exact-query-value-keys "${LAYOUT_EXACT_QUERY_VALUE_KEYS}")
...
extra_args+=(--layout-template-prompt-dedup-fallback-min-fraction "${LAYOUT_TEMPLATE_PROMPT_DEDUP_FALLBACK_MIN_FRACTION}")
extra_args+=(--layout-template-min-saved-call-pages "${LAYOUT_TEMPLATE_MIN_SAVED_CALL_PAGES}")
