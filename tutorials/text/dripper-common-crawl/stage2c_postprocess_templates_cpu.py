...
"""Stage 2c: CPU response parsing, content extraction, and representative template build."""
...
POSTPROCESS_STATUS_COL = "stage2c_status"
POSTPROCESS_ERROR_COL = "stage2c_error"
OUTPUT_FORMAT = "mm_md"
...
OUTPUT_SCHEMA = pa.schema(
    [
        ...
        ("mapping_json", pa.string()),
        ("mapping_error", pa.string()),
...
def _output_path(output_dir: Path, prompt_path: Path) -> Path:
    stem = prompt_path.stem.removeprefix("prompt_")
    return output_dir / f"stage2c_{stem}.parquet"
...
class ResponsePostprocessStage(ProcessingStage[FileGroupTask, DocumentBatch]):
    name: str = "stage2c_postprocess_templates"
    ...
    def process(self, task: FileGroupTask) -> DocumentBatch:
        if self._bindings is None or self._web_bindings is None:
            self.setup()
        ...
        metrics = {
            ...
            "mapping_ok": int(sum(1 for row in output_rows if row.get("mapping_json"))),
            "elapsed_s": round(time.perf_counter() - t0, 3),
        }
        metrics.update({f"status_{key}": int(value) for key, value in status_counts.items()})
        logger.info(
            "stage2c {} rows={} ok={} errors={} mapping_ok={} -> {}",
            prompt_path.name,
            metrics["output_rows"],
            metrics["ok_rows"],
            metrics["error_rows"],
    ...
    def _process_row(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        if prep_status != "ok":
            error = _as_str(row.get(PREPROCESS_ERROR_COL)) or "Stage 2a did not produce a prompt"
            return {
                **base,
                "dripper_error": error,
                POSTPROCESS_STATUS_COL: prep_status or "stage2a_error",
                POSTPROCESS_ERROR_COL: error,
            }
        llm_status = _as_str(row.get(LLM_STATUS_COL))
        if not llm_status or llm_status == "nan":
            error = "missing Stage 2b response"
            return {**base, "dripper_error": error, POSTPROCESS_STATUS_COL: "missing_response", POSTPROCESS_ERROR_COL: error}
        if llm_status != "ok":
            error = _as_str(row.get(LLM_ERROR_COL)) or "Stage 2b response error"
            return {
                **base,
                "dripper_error": error,
                POSTPROCESS_STATUS_COL: "stage2b_error",
                POSTPROCESS_ERROR_COL: error,
            }
        ...
        if _as_str(row.get("cluster_role")) == "representative":
            mapping_result = build_layout_mapping_data(out, bindings=self._bindings, web_bindings=self._web_bindings)
            out["mapping_json"] = serialize_mapping_data(mapping_result.mapping_data)
            out["mapping_error"] = mapping_result.error
        return out
...
def _base_output(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "dripper_simplified_html": _as_str(row.get("dripper_simplified_html")),
        "dripper_mapped_html": _as_str(row.get("dripper_mapped_html")),
...
def run(args: argparse.Namespace) -> None:
    ...
    if not tasks:
        raise RuntimeError(f"No Stage 2c tasks for shard {args.shard_index}/{args.num_shards}")

    logger.info("Stage 2c scheduling {} prompt shard(s)", len(tasks))
    _init_ray_from_slurm()
    stage = ResponsePostprocessStage(cpus_per_actor=args.cpus_per_actor)
    pipeline = Pipeline(name="stage2c_postprocess_templates")
    ...
    summary = {
        ...
        "mapping_ok": int(sum(item.get("mapping_ok", 0) for item in metrics)),
        "shards": metrics,
    }
    summary_path = output_dir / "_stage2c_summary.json"
    summary_path.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    logger.info(
        "Stage 2c done in {:.1f}s rows={} ok={} errors={} mapping_ok={} summary={}",
        elapsed,
        summary["output_rows"],
        summary["ok_rows"],
        summary["error_rows"],
...
def main() -> int:
    ...
    parser.add_argument("--prompts", required=True, help="Stage 2a prompt directory")
    parser.add_argument("--responses", required=True, help="Stage 2b response directory")
    parser.add_argument("--output", required=True, help="Stage 2c output directory")
