...
The input is a Parquet dataset with a ``content`` column holding raw or
independently Zstandard-compressed HTML and a ``url`` column. The output is the same rows plus a
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--html-compression", default="none", choices=["none", "zstd"])
