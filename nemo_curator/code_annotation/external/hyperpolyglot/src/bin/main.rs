...
use hyperpolyglot::{generated::is_generated_io, get_language_breakdown, Detection, Language, LanguageType};
        ...
        if options.matches_filter(language) {
            ...
            if !options.condensed_output {
                for (_, file) in breakdowns.iter() {
                    ...
                    writeln!(stdout, "{} (generated={})", path.display(), is_generated_io(file).map_or("unknown", |r| if r { "yes" } else { "no" }))?;
                }
                writeln!(stdout, "")?;
            }
        }
