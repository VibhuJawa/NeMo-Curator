...
from django.db.models.fields.related import ManyToOneRel, OneToOneField, add_lazy_relation
from django.db.models.loading import get_model, register_models
...
class Model:
    ...
    def _get_next_or_previous_in_order(self, is_next):
        ...
        if not hasattr(self, cachename):
            ...
            obj = (
                self._default_manager.filter(**{order_field.name: getattr(self, order_field.attname)})
                .filter(
                    **{
...
def method_get_order(ordered_obj, self):
    ...
    return [r[pk_name] for r in ordered_obj.objects.filter(**{order_name: rel_val}).values(pk_name)]
...
def get_absolute_url(opts, func, self, *args, **kwargs):
    return settings.ABSOLUTE_URL_OVERRIDES.get("%s.%s" % (opts.app_label, opts.module_name), func)(
        self, *args, **kwargs
    )
