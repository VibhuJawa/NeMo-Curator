...
class MinerUHtmlSimplifyStage(ProcessingStage[DocumentBatch, DocumentBatch]):
    ...
    def setup(self, _worker_metadata: WorkerMetadata | None = None) -> None:
        ...
        self._html_decoder = HtmlCellDecoder(self.html_compression)
    ...
    def _simplify_one(self, raw: str | bytes) -> tuple[str, str, int, str]:
        ...
        try:
            html_str = self._html_decoder(raw)
        except ValueError as e:
            logger.debug(f"HTML decompression failed: {e}")
            return "", "", 0, "decompression_error"
        if not html_str:
...
class MinerUHtmlExtractStage(ProcessingStage[DocumentBatch, DocumentBatch]):
    ...
    def _prune(
        ...
        for i, status in enumerate(statuses):
            if status != "ok":
                main_htmls.append(self._fallback(raw_htmls[i]))
                continue
            try:
    ...
    def _fallback(self, raw_html: str | bytes | None) -> str:
        try:
            return self._fallback_html(self._html_decoder(raw_html))
        except ValueError as e:
            logger.debug(f"HTML decompression failed during fallback: {e}")
            return ""
