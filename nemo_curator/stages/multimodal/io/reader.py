...
class WebDatasetReaderStage(BaseMultimodalReaderStage):
    ...
    def read_tables_and_metadata(self, source_path: str) -> tuple[pa.Table, pa.Table]:
        """Read one tar shard and emit normalized multimodal data + metadata tables."""
        ...
        with open_tar_path(source_path, self.storage_options) as tf:
            for member in tf:
                ...
                try:
                    ...
                    self._handle_member_error(member_name, err)
        data_table = pa.Table.from_pylist(rows, schema=MULTIMODAL_SCHEMA)
        metadata_table = pa.Table.from_pylist(state.metadata_rows, schema=METADATA_SCHEMA)
        return data_table, metadata_table

    def _should_read_member_payload(self, member_name: str) -> bool:
        """Return whether member bytes are required for row construction."""
    ...
    def _handle_member_error(self, member_name: str, err: Exception) -> None:
        """Apply configured member error policy."""
    ...
    def _rows_from_member(
        ...
        """Normalize one tar member into zero or more multimodal rows."""
        suffix = Path(member_name).suffix.lower()
    ...
    def _rows_from_text_member(
        ...
        """Build rows from `.txt`/`.json` members."""
        if suffix == ".json":
            if payload is None:
        ...
        if suffix == ".json":
            # Metadata policy: first JSON payload wins for each sample_id.
    ...
    def _rows_from_interleaved_json(
        ...
        """Decode interleaved JSON metadata into normalized rows."""
        ...
        for idx, segment in enumerate(segments):
            ...
            if not self._loads_modality(modality):
                continue
            if modality == "text":
                text_value = _required_segment_str(segment, text_field)
                rows.append(
                    self._text_row(
                        sid=sample_id,
                        position=idx,
            ...
            rows.append(self._image_row(sid=sample_id, position=idx, source=source, content_key=content_key))
        return rows
    ...
    def _rows_from_binary_member(
        ...
        """Build row for non-text member types."""
    ...
    def _next_sample_and_position(
        ...
        """Resolve sample position for one member and advance fallback counters."""
    ...
    def _loads_modality(self, modality: str) -> bool:
        """Return whether one modality should be emitted."""
...
class WebDatasetReader(CompositeStage[_EmptyTask, MultimodalBatch]):
    ...
    Decomposes into:
    1. ``FilePartitioningStage`` to group input shard paths
    2. ``WebDatasetReaderStage`` to parse each file group

    This is the user-facing entry point for reading WebDataset shards.

    Args:
        file_paths: Input path or list of paths to WebDataset shards.
        files_per_partition: Number of files per partition task.
        blocksize: Target partition size; used when ``files_per_partition`` is not set.
        file_extensions: Extensions to include while partitioning input paths.
        limit: Optional maximum number of file-group partitions to emit.
        load_binary: Forwarded to ``WebDatasetReaderStage``.
        sample_format: Forwarded to ``WebDatasetReaderStage``.
        error_handling: Forwarded to ``WebDatasetReaderStage``.
        modalities_to_load: Forwarded to ``WebDatasetReaderStage``.
        interleaved_field_map: Forwarded to ``WebDatasetReaderStage``.
        max_batch_bytes: Forwarded to ``WebDatasetReaderStage``.
        storage_options: fsspec options forwarded to both partitioning and reader stages.
    """
