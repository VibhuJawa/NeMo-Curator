...
def _collect_sample_rows(
...
) -> tuple[dict[str, Any], dict[int, object], dict[int, tuple[object, object]], dict[str, dict]]:
    """Parse sample rows into per-position maps and metadata payload."""
    payload: dict[str, Any] = {}
    text_at_pos: dict[int, object] = {}
    image_at_pos: dict[int, tuple[object, object]] = {}
    extras: dict[str, dict] = {"text": {}, "image": {}, "metadata": {}}
    ...
    for _, row in sample_df.iterrows():
        ...
        if mod == "metadata":
            ...
            extras["image"][pos] = row_extra

    return payload, text_at_pos, image_at_pos, extras
...
def _write_sample(
    ...
    payload, text_at_pos, image_at_pos, extras = _collect_sample_rows(sample_df, extra_columns)
    ...
    if extra_columns:
        payload["_row_extra"] = {
            "text": [extras["text"].get(pos) for pos in range(n)],
            "image": [extras["image"].get(pos) for pos in range(n)],
