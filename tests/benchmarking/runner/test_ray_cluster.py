...
def test_single_node_allocation_uses_ray_client(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("SLURM_NNODES", "1")
    assert select_ray_client_class() is RayClient


def test_multi_node_allocation_uses_slurm_ray_client(monkeypatch: pytest.MonkeyPatch) -> None:
