...
def test_reader_interleaved_positions_do_not_overlap(tmp_path: Path) -> None:
    ...
    reader = InterleavedWebdatasetReaderStage(source_id_field="pdf_name", sample_id_field="pdf_name")
    df = _as_df(reader.process(task))
...
def test_reader_empty_output_schema_includes_requested_passthrough_fields(tmp_path: Path) -> None:
    ...
    reader = InterleavedWebdatasetReaderStage(source_id_field="pdf_name", fields=("p_hash",))
    df = _as_df(reader.process(task))
    assert "p_hash" in df.columns
...
def test_reader_fields_reserved_key_raises(tmp_path: Path) -> None:
    ...
    reader = InterleavedWebdatasetReaderStage(source_id_field="pdf_name", fields=("sample_id",))
...
def test_reader_max_batch_bytes_splits(tmp_path: Path) -> None:
    ...
    reader = InterleavedWebdatasetReaderStage(source_id_field="pdf_name", max_batch_bytes=1)
    result = reader.process(task)
...
def test_reader_non_list_field(
    ...
    df = _as_df(InterleavedWebdatasetReaderStage(source_id_field="pdf_name").process(task_for_tar(tar_path)))
...
def test_reader_uses_stem_as_sample_id(tmp_path: Path) -> None:
    ...
    reader = InterleavedWebdatasetReaderStage(source_id_field="pdf_name", sample_id_field=None)
    df = _as_df(reader.process(task))
    assert (df["sample_id"] == "my_custom_name").all()


def test_reader_source_id_field_optional(tmp_path: Path) -> None:
    """source_id_field=None (default) does not raise and passes all fields through."""
    payload = {"pdf_name": "doc.pdf", "url": "https://example.com", "texts": ["hi"], "images": []}
    tar_path = write_tar(
        tmp_path / "no_source_id.tar",
