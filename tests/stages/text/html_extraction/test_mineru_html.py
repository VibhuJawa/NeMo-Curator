...
class TestSimplifyStage:
    ...
    def test_accepts_independently_zstd_compressed_html(self) -> None:
        compressed = pa.Codec("zstd").compress(PAGE.encode()).to_pybytes()
        stage = MinerUHtmlSimplifyStage(html_field="_html_zstd", html_compression="zstd")
        stage.setup()
        batch = DocumentBatch(dataset_name="t", data=pd.DataFrame({"_html_zstd": [compressed]}))
        out = stage.process(batch).to_pandas()
        assert out[STATUS_FIELD].iloc[0] == "ok"
        assert "bake bread" in out[MAP_HTML_FIELD].iloc[0]
        assert out["_html_zstd"].iloc[0] == compressed

    def test_invalid_zstd_payload_is_isolated_to_its_row(self) -> None:
        stage = MinerUHtmlSimplifyStage(html_field="_html_zstd", html_compression="zstd")
        stage.setup()
        batch = DocumentBatch(dataset_name="t", data=pd.DataFrame({"_html_zstd": [b"not-zstd"]}))
        assert stage.process(batch).to_pandas()[STATUS_FIELD].iloc[0] == "decompression_error"
...
class TestExtractStage:
    ...
    def test_fallback_decompresses_one_zstd_document(self, simplified: pd.DataFrame) -> None:
        df = simplified.copy()
        df["_html_zstd"] = [pa.Codec("zstd").compress(PAGE.encode()).to_pybytes()]
        df[STATUS_FIELD] = ["too_long"]
        out = self._run(
            df,
            "",
...
class TestServerInferenceRequests:
    ...
    def test_regex_uses_dynamo_guided_regex_field(self) -> None:
        stage, built = self._stage()
        stage.process(self._batch(["a"]))
        assert built[0].calls[0]["guided_regex"] == (r"<answer>\s*1(main|other)2(main|other)\s*</answer>")
...
class TestComposite:
    ...
    def test_zstd_setting_reaches_both_cpu_stages(self) -> None:
        simplify, _, extract = MinerUHtmlExtractor(
            base_url=SERVER_URL,
            html_field="_html_zstd",
            html_compression="zstd",
        ).decompose()
